...
export default function EnquireSettings() {
  ...
  // ✅ drafts for options textarea (one per field.id)
  ...
  const updateField = (idx: number, patch: Partial<FormField>) => {
    setConfig((prev) => {
      ...
      // if switching away from options type, clear options + draft
  ...
  const validateAll = () => {
    // ensure option drafts are committed
            ...
            return (
              <div key={field.id ?? idx} className={styles.fieldRow}>
                <div className={styles.fieldHeader}>
                  ...
                  <button type="button" onClick={() => moveField(idx, idx - 1)} disabled={idx === 0}>
                    ↑
                  </button>
                  <button
                    type="button"
                ...
                <div className={styles.fieldGrid}>
                  <label className={styles.full}>
                    ...
                    />
                    <small>Example: enq_type=Booking (field appears only for Booking enquiries).</small>
                  </label>
                </div>
